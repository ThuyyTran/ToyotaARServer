...
from getHistRerank import genHist
...
def insert_file(files):
  logger.info('Start insert data to numpy')
  ...
  for obj in files:
    for productId in obj:
      add_files = obj[productId]
      add_files_fullpath = [os.path.join(_config.img_db_path, item) for item in add_files]
      crop_files, error_files = generate_boxs.images2bbxs(add_files_fullpath, _config.img_db_path, CROP_IMAGE_FOLDER)
      logger.error(f'Crop file error: {error_files}')

      add_list.extend(crop_files)
      master_data_paths.extend([f"{item.replace(f'{CROP_IMAGE_FOLDER}/', '')},{productId}" for item in crop_files])

  # generate hist
  gen_histogram(add_list)
...
def gen_histogram(files):
  logger.info('Start generate histogram')
  for i in tqdm(range(len(files))):
    f = str(files[i]).replace(f'{CROP_IMAGE_FOLDER}/', '')
    sub_path = os.path.dirname(f)
    sub_path = os.path.join(_config.hist_db_path, sub_path)
    pathlib.Path(sub_path).mkdir(parents=True, exist_ok=True)

    input_file = files[i]
    output_file = os.path.join(_config.hist_db_path, f + ".pkl")

    genHist(input_file, output_file)
  ...
  for i in range(len(jobs)):
    for key in jobs[i]:
      ...
      if job_status == "ADD" or job_status == 'UP':
        files_insert.append({ key: job_files })
...
def process_job(jobContent):
  ...
  error_list.clear()
  add_list.clear()
  files_insert = get_file_paths(jobContent)

  # update features index
  ret = insert_file(files_insert)
...
def main():
  ...
  while True:
    ...
    query_id = bytes('3', 'utf-8')
    if query_id is not None:
      t0 = datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")
      query_id = query_id.decode('utf-8')

      # get job content
      job_key = "%s_%s_manual_job" % (settings.APP_DB_NAME, str(query_id))
...
def send_response(ret, msg=""):
  ...
  # begin send request
  UPDATE_STATUS_API = f"/api/{app_code}/upload_manual_task/{current_job}"
...
class Config:
  def __init__(self, db_config):
    ...
    self.hist_db_path = db_config[settings.MTC_HIST_DB_FOLDER_KEY]
...
if __name__ == "__main__":
  ...
  NPY_FOLDER = '/media/anlab/data-1tb1/revamp_20220413/npy'
  CROP_IMAGE_FOLDER = '/media/anlab/data-1tb1/revamp_crop'

  print(f'app_code: {app_code} - npy: {NPY_FOLDER}')

  # initialize model
  model = CNN(useRmac=True, use_solar=True, transform_ratio=300)
